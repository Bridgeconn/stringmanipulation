...
'''The function has a array with book standard name and nummber'''
...
'''Renames the files in this folder'''
os.chdir("Target")
files = os.listdir()

'''Fetch each file from the list'''
for file in files:
    print(file)

    '''The file is being rename and numbered here'''
    try:
        if(file.split(".")[-1].lower()=="usfm" or file.split(".")[-1].lower()=="sfm"):
            f=codecs.open(file, mode='rb', encoding="utf-8")
            fc = f.read()
            f_id=re.search("\\\\id (.{3})", fc, re.U)
