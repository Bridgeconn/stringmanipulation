'''
 Convert folders (book,chapters) and files from a main folder to an excel file. 
'''
...
def complete_note(rows, book_name, chapter_number, verse_number, versus, x):
...
def book_update(rows,book_name,chapter,verse,note,file_name):
	work_book = load_workbook(file_name)
	worksheet = work_book.get_sheet_by_name('Sheet')
	work_book.active
...
for book_folder in folder:
	...
	for chapter_folder in sort_folder:
		...
		'''Fetching .md files'''	
		chapter_folder = sorted(glob.glob("*.md"))
		
		#chapter_folder1 = chapter_folder[-1]
		if chapter_folder[-1].lower() ==	"intro.md":
			verse_number = chapter_folder[-1]
			versus = re.sub(r".md","",verse_number)
			x = 2
			complete_note(rows, book_name, chapter_number, verse_number, versus, x)
			rows += 1
			...
			for verse_number in sort_verse:
				search_digit = re.match(r"(\d\d?).",verse_number)
				if search_digit:
					verse_number = search_digit.group(1) + ".md"
